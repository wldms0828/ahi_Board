...
public class PagiNavigation {
	private String root; // root-context -> (/board)
	private boolean nowFirst; // 10페이지 씩 설정할 때 if문의 조건이 된다. ex) 지금 7페이지면 이전 버튼 누를 수 없게
	...
	public void makeNavigator() {
		...
		// StringBuffer 쓰는 이유는?
		// 속도 차이가 많이 나기 때문에 많은 양을 사용할 때 StringBuffer 클래스를 사용한다.
		
		int pageSize = BoardConstance.PAGE_COUNT;
		int preEnd = (pageNo - 1)/pageSize*pageSize; // 이전 단계의 마지막
		...
		if (this.isNowEnd()) {
			...
			tmpNavigator.append("   <label class='mvpage' move-page-no='"+(preEnd+pageSize+1)+"'>다음</label>");
